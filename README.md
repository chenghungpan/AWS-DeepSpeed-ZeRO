...
ZeRO optimizes memory usage in a data-parallel setting by partitioning the states of the optimizer, gradients, and parameters across the GPUs. This partitioning allows for significantly larger models to be trained, as no single GPU holds the entire model state, thus alleviating the memory bottleneck. <br>
